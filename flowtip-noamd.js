...
(function (namespace, jquery, underscore, react) {
  ...
  flowtip_content = function (_, React) {
    return React.createClass({
      render: function () {
        var classNames = _.chain([
          'flowtip-content',
  ...
  flowtip_root = function ($, _, React, FlowTipTail, FlowtipContent) {
    return React.createClass({
      ...
      render: function () {
        ...
        var contentProperties = { className: this.props.contentClassName };
        var tailProperties = _.extend({ className: this.props.tailClassName }, _.pick(this.props.tail, [
          'top',
          'left',
          'width',
          'height',
        ...
        return React.createElement('div', {
          ...
          className: classNames,
          ref: 'root'
        }, React.createElement(FlowtipContent, React.__spread({}, contentProperties), this.props.children), React.createElement(FlowTipTail, React.__spread({ ref: 'tail' }, tailProperties)));
      }
    });
  }(jquery, underscore, react, flowtip_tail, flowtip_content);
  flowtip = function ($, _, React, FlowTipRoot) {
    return React.createClass({
      ...
      availableAndFitsIn: function (regions, regionParameter, _first) {
        ...
        var availables = regionParameter[region].availables, fits = regionParameter[region].fits, hides = this.props.hideInDisabledRegions;
      ...
      rootPivot: function (region, targetParameter, rootDimension) {
        ...
        if (rootAlign == 'center') {
          ...
          if (region == 'top' || region == 'bottom') {
            pivots = [
              targetPivot,
              targetPivot - rootDimension.width
            ];
          } else if (region == 'left' || region == 'right') {
            pivots = [
              targetPivot,
              targetPivot - rootDimension.height
            ];
          }
      ...
      calculateRegion: function (parent, target) {
        ...
        if (_.contains([
            'top',
            'bottom'
          ], region) && !regionParameter.top.fits && !regionParameter.bottom.fits) {
          region = this.availableAndFitsIn([
            'left',
            'right'
          ], regionParameter);
            ...
            'left',
            'right'
          ], region) && !regionParameter.left.fits && !regionParameter.right.fits) {
          region = this.availableAndFitsIn([
            'top',
            'bottom'
          ], regionParameter);
      ...
      render: function () {
        ...
        return React.createElement('div', {
          style: style,
          clgassName: 'flowtip'
        }, React.createElement(FlowTipRoot, React.__spread({ ref: 'root' }, rootProperties), this.props.children));
      }
    });
  }(jquery, underscore, react, flowtip_root);
  namespace.FlowTip = flowtip;
}(window, $, _, React));
}());